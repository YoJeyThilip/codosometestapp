	...
	<table id="dt_basic" class="table table-striped table-hover dataTable no-footer" width="100%" role="grid" aria-describedby="dt_basic_info" style="width: 100%;">
		<thead>
			<tr role="row">
				...
				<th class="sorting_asc" tabindex="0" aria-controls="dt_basic" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Color Name: activate to sort column ascending" style="width: 207px;">Back</th>
	...
	<script>
		$(document).ready(function() {
			...
			$('#dt_basic').dataTable({
						...
						// custom toolbar
						$("div.dt-toolbar .create-button-colors-style").html('<form action="{{ route("calculator-tables") }}?insert=calculator_fabric" method="post">@csrf<input type="hidden" name="insert_page" value="calculator_fabric"><input type="submit" class="insert_form" value="Create New"></form>');  
