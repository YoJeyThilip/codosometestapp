									...
									<div class="next-field__connected-wrapper">
									 ...
									 <div class="next-form next-form--full-width next-field--connected--no-border-radius">
										...
										<div class="next-input--stylized next-field--connected">
										   ...
										   <input type="text" name="query" id="query" placeholder="Search orders" bind="query" class="next-input next-input--invisible">
										</div>
									 </div>
									</div>
								</form>
                     ...
                     <div class="ui-card__section">
                        <div class="ui-type-container">
                           <div class="table-wrapper">
                              <table class="table-hover expanded" define="{
                                 bulkOperations: new Shopify.BulkOperations({
                                 path: &quot;\/admin\/orders\/set&quot;,
                                 parameterName: 'order_ids' }, this)
                                 }">
                                 <thead>
                                    <tr>
                                       <th class="is-sortable @if( $sortby == 'order_id' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=order_id&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Invoice</a>
										</span>
                                       </th>
                                       <th class="is-sortable @if( $sortby == 'due_date' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=due_date&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Due date</a>
										<span>
                                       </th>
                                       ...
                                       <th class="is-sortable @if( $sortby == 'customer' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=customer&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Customer</a>
										</span>
                                       </th>
                                       ...
                                       <th class="is-sortable @if( $sortby == 'payment_status' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=payment_status&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Payment status</a>
										</span>
                                       </th>
                                       <th class="is-sortable type--right @if( $sortby == 'order_total' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=order_total&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Job Amount</a>
										</span>
                                       </th>
                                       ...
                                       <th class="is-sortable @if( $sortby == 'payed' ) @if( $sortway == 'ASC' ) sorted-desc @else sorted-asc @endif @endif">
										<span>
                                          <a href="{{ route('orders.index') }}?sortby=payed&sortway=@if( $sortway == 'ASC' ){{ 'DESC' }}@else{{ 'ASC' }}@endif">Payed</a>
										</span>
                                       </th>
                                    </tr>
                                 </thead>
								  ...
								  @if ( $printavo_status == "connected"  )
										@foreach( $orders as $order )
											<tr class="ui-nested-link-container ">
											   ...
											   <td class="type--right total next-table__cell--top-right-when-condensed">{{ $order->order_total }}</td>
							...
							<div class="next-grid next-grid--no-padding next-grid--center-aligned">
							   <div class="next-grid__cell next-grid__cell--no-flex">
								  <ul id="pagination-links" class="segmented" refresh-always="">
									 <li>
										<a class="btn js-keep-open js-pagination-link js-prev-btn tooltip tooltip-bottom @if( $prev_page == '' ) disabled @endif" href="{{ route('orders.index') }}?page={{ $prev_page }}@if( isset($query) )&query={{ $query }}@endif">
									 ...
									 <li>
										<a class="btn js-keep-open js-pagination-link js-next-btn tooltip tooltip-bottom @if( $next_page == '' ) disabled @endif" href="{{ route('orders.index') }}?page={{ $next_page }}@if( isset($query) )&query={{ $query }}@endif">
										   <span class="tooltip-container">
